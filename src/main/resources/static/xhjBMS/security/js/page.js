...
(function($){
    ...
    $.fn.asynPage = function(url,contentContainer,buildHtml_fun,usePager,loadService,params,callerSettings,contentContainer1){  
        ...
        paramStr = makeParamStr(settings.param,settings);
        if(loadService){
        	//开始获取数据  
        	fetchData(page.getPageNO(), false, settings);  
        }else if(loadService==false && usePager){
        	//不加载数据，显示分页信息
        	page = new Page(1,10,0);  
        	buildPageWidget(page, false, settings);//触发构建分页控件
        }
    ...
    /* 将json转换为请求参数字符串 */  
    var trunParamConfig2RequestParamStr = function(json, settings){  
        var str = "";  
        var pageNo;
        var pageSize ;
        var jsonstr="{";
        for(key in json){  
        	if(key == 'pageNo'){
        		pageNo = json[key];
        		jsonstr+="\"VIEW_INDEX\":\""+json[key]+"\",";
        	}else if(key == 'pageSize'){
        		pageSize = json[key];    
        		jsonstr+="\"VIEW_SIZE\":\""+json[key]+"\",";
        	}else if(jsonstr=="{"){  
        ...
        jsonstr+="}";
//        if(!pageNo){
//        	str+= "&page="+settings.pageNo;
//        }
//        
//        if(!pageSize){
//        	str+= "&pagesize="+settings.pageSize;
//        }
    ...
    /* 将配置参数拼接为请求字符串 */  
    var makeParamStr = function(param_json,settings){  
        if(param_json==null){  
            return "";  
        }else{  
            return trunParamConfig2RequestParamStr(param_json,settings);  
        }  
    };  
     ...
     */  
    var fetchData = function(pageNo,isToPage, settings){  
    	if(isToPage == true){
        	settings.pageNo = 1;
...
//    	alert(JSON.stringify(settings.param))
        var data = makeParamStr(settings.param,settings); 
//    	alert(JSON.stringify(data))
        $.ajax({  
            ...
            success :function(datas){
                ...
                page = new Page(jsonData.viewIndex,jsonData.viewSize,jsonData.listSize);
                settings.buildHtml_fun(jsonData.pageData);  
                if(settings.usePager){
                	buildPageWidget(page,settings);//触发构建分页控件
                }
            },  
            error:function(xmlHttpRequest,textStatus,errorThrown){
                if(textStatus == "error"){  
                    var error = eval('('+xmlHttpRequest.responseText+')');  
                    console.log("Sorry："+error.errorCode+"，"+error.message+"！");  
                }  
            }  
        });  
    };  
      
    var trunTargetPage = function(pageNum,isToPage,settings){  
        fetchData(pageNum,isToPage,settings);  
    }  
        ...
        $.each(links,function(i,link){  
            ...
            link.click(function(){  
            	 ...
            	 if(pageNum && pageNum >0){
            		 trunTargetPage(pageNum,false,settings);  
            	 }else if(toPage && toPage>0){
            		 trunTargetPage(toPage,true,settings);
            	 }
            });
        });  
    }  
    ...
    /* 构建分页控件的具体算法实现 */  
    var buildPageWidget = function(page,settings){  
        ...
        /* --------------- 下面开始进入真正的分页控件构建过程 -------------- */ 
        settings.pageWidget.append("<div class='pull-right' style=' line-height:32px; padding-right:15px;'>共有 "+page.getTotalData()+" 条数据，当前第 "+pageNo+" 页</div>");  
        settings.pageWidget.append("<div class='pull-left'>");
        settings.pageWidget.find('div').next().append("<span class='pagination pagination-centered'><ul class='pagination'></ul></span>");
        ...
        settings.pageWidget.find('div').next().append('<span class="btn-group dropup" style="padding-right:100px;"></span>');  
        /* --------------- 5构建 ------------- */  
        settings.pageWidget.find('span').next().append('<button class="btn dropdown-toggle" data-toggle="dropdown">'+ pageSize +'<span class="caret"></span></button>');
